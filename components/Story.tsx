...
const Story = () => {
		...
		const animate = (now: number) => {
			...
			const updated = stats.map((stat) => {
				...
				if (stat.target <= 10) {
					...
					return Math.min(
						stat.target,
						Math.floor(progress * (stat.target + 1)),
					);
				}
	...
	return (
		<section className="relative isolate overflow-hidden border-y border-[#00ff87]/10 bg-gradient-to-l from-[#2e3428] via-[#000000] to-[#2e3428] px-4 py-20 text-white sm:px-6 lg:py-28">
			
			<div className="absolute inset-0 -z-10 bg-[linear-gradient(180deg,rgba(5,8,6,0.18)_0%,rgba(5,8,6,0.58)_38%,rgba(5,8,6,0.82)_100%)]" />
					...
					<div className="flex flex-col justify-center gap-10 sm:gap-12">
						...
						<div
							...
							{stats.map(({ label, suffix, target, delay }, index) => {
								const isCompleted =
									displayStats[index] === target && metricsInView;
								return (
									<div
										key={label}
