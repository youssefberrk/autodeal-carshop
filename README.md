...
### AI Car Concierge
* **Grounded AI Assistant**: A premium automotive concierge that helps users discover and compare vehicles based on real catalog data.
* **Contextual Recommendations**: Provides vehicle suggestions based on user budget, performance needs, and style preferences.
* **Real-time Tool Integration**: Uses OpenAI tool calling to search and recommend vehicles from the authoritative `carsData` catalog, preventing hallucinations.
* **Context-Aware Entry Points**: Integrated into the Navbar, Shop page, and Vehicle Details pages to provide seamless assistance.
...
│   ├── CarConcierge.tsx
│   ├── ConciergeInput.tsx
│   ├── ConciergeMessage.tsx
│   └── CarRecommendationCard.tsx
├── cards/
├── checkout/
├── navbar/
├── sections/
...
* Integrating third-party APIs and services (OpenAI, Stripe, Resend)
